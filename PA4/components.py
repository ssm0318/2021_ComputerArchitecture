...
class Memory(object):
    ...
    # dmem should be accessed using access2()
    def access2(self, valid, addr, data, fcn, pc, stage):
        ...
        if ((stage == MM_STAGE1 and self.s1_pc != WORD(0xffffffff)) or
            (stage == MM_STAGE2 and self.s1_pc != pc)):
            print('<dmem conflict>')
            return ( WORD(0xffffffff), False )

        if stage == MM_STAGE1:
            self.s1_pc = pc
            if (addr < self.mem_start) or (addr >= self.mem_end) or \
                addr % self.word_size != 0:
                return ( WORD(0), False )
            else:
                return ( WORD(0), True )
        elif stage == MM_STAGE2:
            if (addr < self.mem_start) or (addr >= self.mem_end) or \
                addr % self.word_size != 0:
                return ( WORD(0), False )
            elif fcn == M_XRD:
...
#--------------------------------------------------------------------------
#   RAS: models an n-entry return address stack
#--------------------------------------------------------------------------
